...
NSString *const BU_PrimeRitKey = @"bu_PrimeRitKey";
NSString *const BU_ShowSeqKey = @"bu_ShowSeqKey";
NSString *const BU_ShowSeqTimestamp = @"bu_ShowSeqTimestamp";
...
- (void)viewDidLoad {
    ...
    if (!self.adInfo) {
        ...
        [self.adInfo setObject:@"1" forKey:BU_PrimeRitKey];
    }
    [self loadData];
}
...
- (void)loadData {
    NSString *primeRit = [self.adInfo objectForKey:BU_PrimeRitKey];
    if (!primeRit) {
        primeRit = @"1";
        [self.adInfo setObject:@"1" forKey:BU_PrimeRitKey];
    }
    NSInteger showSeq = [[self.adInfo objectForKey:BU_ShowSeqKey] integerValue];
    
    NSInteger timestamp = [[self.adInfo objectForKey:BU_ShowSeqTimestamp] integerValue];
    if (!timestamp) {
        [self.adInfo setObject:@([self nowTimestemp]) forKey:BU_ShowSeqTimestamp];
    }else{
        if (timestamp > [self zeroOfToday]) {
            NSInteger showSeqNum = showSeq;
            [self.adInfo setObject:@(showSeqNum + 1) forKey:BU_ShowSeqKey];
        }else{
            [self.adInfo setObject:@(1) forKey:BU_ShowSeqKey];
        }
    }
    ...
    slot1.showSeq = [[self.adInfo objectForKey:BU_ShowSeqKey] integerValue];
